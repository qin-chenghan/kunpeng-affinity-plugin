...
def _resolve_generic_nodes(numa_utils: Any, platform: Any) -> list[int]:
    ...
    return _nodes_from_generic_batch(resolve_vllm_generic_affinity(platform))
...
def _resolve_automatic_nodes(
    ...
    *,
    ...
    if not force_generic:
        ...
        try:
            return resolve_vllm_native_nodes(numa_utils, platform), "native"
    ...
    return _resolve_generic_nodes(numa_utils, platform), "generic"
...
def _automatic_affinity_context(
    *,
    ...
    try:
        nodes, source = _resolve_automatic_nodes(
            numa_utils,
            _current_platform(),
    ...
    if existing is None:
        ...
        error = AffinityDiscoveryError(
            f"NUMA nodes changed concurrently from resolved {nodes} to {existing}",
            code="CONCURRENT_CONFIG_CONFLICT",
        )
        if force_generic or mode is PluginMode.STRICT:
            raise _strict_failure(error) from error
        logger.warning(
            "[kunpeng-affinity] automatic affinity skipped code=%s: %s",
            error.code,
            error,
...
def register() -> None:
    ...
    def configure_subprocess_wrapper(*args: Any, **kwargs: Any) -> Iterator[None]:
        ...
        if (mode is not PluginMode.OFF or force_generic) and _should_resolve(
